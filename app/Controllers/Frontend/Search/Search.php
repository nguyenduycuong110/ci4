...
class Search extends FrontendController{
    ...
    public function index(){
        $keyword = $this->request->getGet('keyword');
        $keyword = '(tb2.title LIKE \'%'.$keyword.'%\' OR tb2.description LIKE \'%'.$keyword.'%\' OR tb2.content LIKE \'%'.$keyword.'%\' )';
        ...
        if(isset($this->module)){
           foreach($this->module as $key => $val){
              $this->data['search'][$key]['list'] =  $this->AutoloadModel->_get_where([
                'select' => 'tb1.id, tb1.image, tb2.title, tb2.canonical, tb2.description, '.(($key == 'product') ? 'tb1.price' : '').'',
                'table' => $key.' as tb1',
                'join' => [
                    [
                       $key.'_translate as tb2' ,'tb1.id = tb2.objectid','inner'
                    ]
                ],
                'where' => [
                    'tb1.publish' => 1,
              ...
              $this->data['search'][$key]['heading'] = $val;
              
           }   
        }
        $this->data['item'] = $this->data['search'];
        $this->data['general'] = $this->general;
        $this->data['keyword'] = $this->request->getGet('keyword');
// dd($this->data['item']);
        $this->data['template'] = 'frontend/search/index';
        return view('frontend/homepage/layout/home', $this->data);
